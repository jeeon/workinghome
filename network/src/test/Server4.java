	...
	void startServer() {
		try {
			...
			while (true) {
				...
				new KBBCom_Thread(socket).start();										// 클라이언트와 통신하는 스레드를 생성하고 실행시킨다.


				sManager.add(socket);													// 소켓 관리자 리스트에 소켓을 추가한다.


				System.out.println("현재 클라이언트 수: " + sManager.size());					// 현재 접속하고 있는 클라이언트의 수를 화면에 출력한다.
			}
		} 
		catch (Exception e) {
	...
	class KBBCom_Thread extends Thread {
		...
		public void run() {
			try {
				...
				reader = new DataInputStream(socket.getInputStream());					// 소켓의 입·출력 스트림을 얻는다.
				writer = new DataOutputStream(socket.getOutputStream());

				String msg;

				while ((msg = reader.readUTF()) != null) {								// 입력 스트림으로부터 메시지를 얻는다.
					if (msg.equals("OK")) { 											// 받은 메시지가 "OK"이면


						writer.writeInt(rnd.nextInt(3));								// 0에서 2사이의 정수형 난수를 클라이언트로 전송한다.
						writer.flush();
					}
				}
			} 
